...
This evidence lane's workspace-member hint is not the full workspace/owner-topology capability: `analysisRun()` keeps its compatible route-only target, and apply binding, sibling-owner rejection, and complete topology remain future work. The existing bounded project Agent Asset inventory is reused for applicable instruction surfaces, but the full Agent Asset Evidence Lane remains incomplete. `projectEvidence()` does not create findings and is not wired into reports, proposals, findings, automation, or extension UI.
...
This API is not wired into reports, findings, automation, the extension UI, or a config feature gate.
