...
The published P1 proposal-to-finding linkage (`fbeeb99`) adds optional `finding_id` and `expected_finding_revision` proposal frontmatter. Bound proposals require a caller-supplied `F-####` finding and an exact current revision; approval and the final pre-apply boundary both fail closed with stable `PROPOSAL_FINDING_INVALID`, `PROPOSAL_FINDING_MISSING`, or `PROPOSAL_FINDING_REVISION_STALE` codes. Rejection does not mutate the finding, multiple proposal references remain possible, and legacy proposals without the fields remain readable and usable. Proposal summaries/history and proposal command responses are reader-safe and omit private paths, raw session references, and evidence content.

The current Findings-backed report candidate builds a bounded `pi-harness.findings-projection` from the reader ledger and embeds its exact Markdown in `report()`. It displays lifecycle status, evidence state, confidence, impact, expected outcome, smallest owner, acceptance checks, proposal references, and reader-safe evidence-ref counts. Active records render before completed records without a score; zero findings and `Missing` remain explicit non-outcome states. The projection is capped at 64 displayed findings with an omission notice, re-applies reader validation/redaction, and is presentation only—the private ledger remains canonical. `/harness` renders the same report Markdown in its Pi TUI dashboard and adds total/active finding counts, so Markdown and TUI do not perform separate analysis.
...
Findings remain caller-supplied and are not wired into detectors, validation receipts, automation, or a config feature gate. Report/TUI wiring reads only the canonical reader ledger and never infers findings or evidence states from session metrics. A proposal binding is checked only when an explicitly bound proposal is approved or applied.
