...
12. **Generate outputs** — report, reflection, reviewed proposal, eval, or automation functions consume normalized session results.
...
- `writeFindings()` and `readFindings()` are additive synchronous APIs for a caller-supplied private ledger only. They do not read user-home assets, raw sessions, or private evidence outside their own Harness ledger, and are not wired to detectors, proposal linkage, reports, validation receipts, automation, or extension UI.
