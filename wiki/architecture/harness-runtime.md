...
8. **Record caller-supplied findings independently** — `writeFindings()` validates explicit schema-1 findings and evidence states, upserts them by stable `F-####` identity, increments finding revisions for changed records, and preserves append-only private history. `readFindings()` returns only the bounded reader projection. This owner does not inspect sessions or assets and is not invoked by detectors or automation. Explicit proposal binding reads the ledger only to validate a caller-supplied binding; `report()` reads it once to build a non-canonical reader-safe Markdown/TUI projection without inferring findings or states.
9. **Optionally build Task Episode candidates** — the explicit `taskEpisodes()` API consumes the frozen run through its own consumer, validates normalized event file order, uses active-path evidence only, creates one non-merged session-bounded candidate per user turn, links observed change/validation sets conservatively, and publishes canonical private plus derived reader artifacts. It is not invoked by reports, findings, proposals, automation, or extension UI.
10. **Review deterministic candidates before proposal** — R-0001 through R-0004 emit proposal-free `CandidateSignal` objects whose private evidence refs include the frozen session `sourceFingerprint` and normalized event ID when available. One reviewer filters by API mode, inventories bounded applicable project `AGENTS.md`/`wiki/**/_rules.md` ancestry, evaluates executable per-block coverage signatures and eligibility, then retains promoted/deferred/rejected decisions.
11. **Begin, write, and finalize one proposal attempt** — before proposal files are touched, the runtime exclusively persists a pending attempt receipt. It then writes/skips promoted proposals and atomically finalizes that same receipt. Begin failure prevents proposal writing; finalize failure leaves the pending receipt intact.
12. **Validate optional proposal linkage** — proposal read/render preserves legacy proposals and optional `finding_id`/`expected_finding_revision` fields. Approval and the final pre-patch apply boundary validate the finding identity and exact revision through `src/proposals/finding-binding.js`; rejection and linkage-free proposals retain their existing behavior.
13. **Generate outputs** — report, reflection, reviewed proposal, eval, or automation functions consume normalized session results.
...
- `src/proposals/finding-binding.js` — explicit finding identity/revision validation for bound proposals
...
- `src/findings/evidence-states.js` — exact executable Evidence State constants and normalization
- `src/findings/schema.js` — schema-1 finding validation, stable identity/dedupe, revisions, and reader-safe record projection
- `src/findings/ledger.js` — private project-key Findings latest/history persistence and fail-closed reader projection
- `src/findings/projection.js` — bounded reader-safe Findings counts/Markdown presentation used identically by report and Pi TUI
...
- `agentAssets()` is the published additive Agent Asset Evidence lane. It inventories project `AGENTS.md`/`wiki/**/_rules.md` ancestry through the existing secure reader, `.pi/agent/model-prompts/*.md`, and explicitly declared project-local `skills`/`extensions` from root `package.json` Pi declarations or `.pi/settings.json`. Its projection contains only project-relative presence/configuration, section IDs, per-surface `complete`/`partial`/`failed` status, bounded counts, and safe diagnostics. It does not inspect user-home assets, undocumented MCP/external package roots, raw sessions, auth, payload logs, `.env`, or private Harness evidence, and it makes no selection/use/exercise/outcome claim.
- Evidence states are exact caller-supplied labels: `Present`, `Wired`, `Exercised`, `Outcome-supported`, `Missing`, `Unobserved`, and `Not-applicable`. The runtime preserves the distinctions between inventory, task wiring, exercise, later comparable outcome, confirmed absence, and an insufficient observation boundary; it performs no score or count inference.
- The published P1 Evidence State + Findings slice owns `writeFindings()`/`readFindings()` under `src/findings/`. It stores validated schema-1 records under the private project key in `findings/latest.json` and append-only `findings/history.jsonl`, uses stable `F-####` identity plus per-finding revisions, supports multiple proposal references without automatic state changes, and fails closed on malformed, unsafe, oversized, or tampered storage. Its public projection excludes absolute paths, private locators, raw evidence, prompts, and commands.
- The published P1 proposal-linkage slice (`fbeeb99`) owns optional binding normalization and validation in `src/proposals/finding-binding.js`. It requires an existing finding at the expected revision for approval and again immediately before apply, with stable machine-testable error codes; it never creates findings or changes finding state.
- The current Findings-backed report candidate owns bounded reader presentation in `src/findings/projection.js`. `report()` reads the canonical Findings ledger once, validates/re-redacts its public records, embeds the exact generated Markdown, and returns counts/projection metadata. The projection shows active records before completed records, preserves exact evidence/lifecycle states without scores, caps presentation at 64 records with an omission notice, and is never canonical state. `/harness` renders that same Markdown and only adds total/active counts to the TUI header.
...
- `writeFindings()` and `readFindings()` are additive synchronous APIs for a caller-supplied private ledger only. They do not read user-home assets, raw sessions, or private evidence outside their own Harness ledger and remain unwired to detectors, validation receipts, or automation. Proposal linkage reads this ledger only for an explicit finding ID/revision binding; report/TUI projection reads only its reader-safe canonical output and performs no finding/state inference.
...
- `report()` consumes unchanged sessions from the frozen selection, writes cache, reads the project Findings ledger once, creates a Markdown project report, and writes `reports/latest.md`. Its JSON payload includes the same frozen session list plus a bounded `pi-harness.findings-projection`; `/harness` therefore makes no second session discovery or Findings analysis call. The report explicitly labels its operational/private boundary, while its Findings section remains reader-safe and omits private locators, raw evidence, prompts, and commands.
...
- Proposal drafts optionally render/read `finding_id` and non-negative `expected_finding_revision`. Bound proposals fail closed at approval and again immediately before apply when the finding is missing or its revision is stale (`PROPOSAL_FINDING_MISSING` / `PROPOSAL_FINDING_REVISION_STALE`); malformed or partial bindings use `PROPOSAL_FINDING_INVALID`. Legacy proposals without both fields remain compatible, rejection does not mutate findings, and public proposal summaries/history omit private paths and evidence content.
...
- `approveProposal()` validates an explicitly bound finding ID and expected revision before changing proposal status. `rejectProposal()` does not validate or mutate the finding.
...
- `applyProposal()` validates a bound finding before branch/apply work and again immediately before text patches, so a finding revision change after approval cannot be applied.
...
- Proposal history is written as JSONL under the private harness project cache, while public history/summary projections omit private locators, raw session references, and evidence content.
