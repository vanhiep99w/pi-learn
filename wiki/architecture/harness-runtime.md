...
- `writeFindings()` and `readFindings()` are additive synchronous APIs for a caller-supplied private ledger only. They do not read user-home assets, raw sessions, or private evidence outside their own Harness ledger, and are not wired to detectors, reports, validation receipts, automation, or extension UI. Proposal linkage reads this ledger only for an explicit finding ID/revision binding.
